...
%     net.divideParam.testRatio = 15/100;
    ...
    e_tr = t(tr.trainInd) - output_tr;
    e_te = t(tr.testInd) - output_te;
       
    %MAE for training and test data
    maetr(i) = mean(abs(output_tr-t(tr.trainInd)));
    maete(i) = mean(abs(output_te-t(tr.testInd)));
    
    %MRE for training and test data
    mretr(i) = mean(abs(output_tr-t(tr.trainInd))/t(tr.trainInd));
    mrete(i) = mean(abs(output_te-t(tr.testInd))/t(tr.testInd));
    ...
    normr_tr = norm(e_tr);
    SSE_tr = normr_tr.^2;
    SST_tr = norm(t(tr.trainInd)-mean(t(tr.trainInd)))^2;
    R_tr(i) = 1 - SSE_tr/SST_tr;
    
    normr_te = norm(e_te);
    SSE_te = normr_te.^2;
    SST_te = norm(t(tr.testInd)-mean(t(tr.testInd)))^2;
    R_te(i) = 1 - SSE_te/SST_te;
    
    %save the network in networks folder
    save(['networks\net' num2str(i)]);  
    elapsed_time (i) = toc;
