...
for i=1:n_hlayer  %vary number of hidden layer neurons from 1 to 100
   ...
   tic 
    ...
    mretr(j) = mean(abs(output_tr-t(tr.trainInd))/t(tr.trainInd));
    mrete(j) = mean(abs(output_te-t(tr.testInd))/t(tr.testInd));
...
%     R_te(j) = regression(t(tr.testInd), output_te);
    ...
    R_tr(j) = 1 - SSE_tr/SST_tr;
    
    normr_te = norm(e_te);
    SSE_te = normr_te.^2;
    SST_te = norm(t(tr.testInd)-mean(t(tr.testInd)))^2;
    R_te(j) = 1 - SSE_te/SST_te;
